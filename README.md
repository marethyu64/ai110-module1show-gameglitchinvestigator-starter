...
3. The attempts counter was initialized to 1 instead of 0, causing the "out of attempts" message to appear one turn too early.
4. The info bar always displayed "Guess a number between 1 and 100" regardless of the selected difficulty.
5. The New Game button used a hardcoded range and did not reset "status", "score", or "history".

Explain what fixes you applied.

1. Swapped the hint messages in "check_guess" so "Too High" directs the player lower and "Too Low" directs them higher.
2. Removed the even/odd type transforming block and always pass the integer secret to "check_guess".
